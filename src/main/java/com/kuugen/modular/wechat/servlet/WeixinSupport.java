package com.kuugen.modular.wechat.servlet;

import com.kuugen.modular.wechat.handle.EventHandle;
import com.kuugen.modular.wechat.handle.MessageHandle;
import com.kuugen.modular.wechat.message.BaseMsg;
import com.kuugen.modular.wechat.message.TextMsg;
import com.kuugen.modular.wechat.message.aes.AesException;
import com.kuugen.modular.wechat.message.aes.WXBizMsgCrypt;
import com.kuugen.modular.wechat.message.req.*;
import com.kuugen.modular.wechat.util.MessageUtil;
import com.kuugen.modular.wechat.util.SignUtil;
import com.kuugen.modular.wechat.util.StrUtil;
...
import static com.kuugen.modular.wechat.util.BeanUtil.isNull;
import static com.kuugen.modular.wechat.util.BeanUtil.nonNull;
import static com.kuugen.modular.wechat.util.CollectionUtil.isEmpty;
import static com.kuugen.modular.wechat.util.CollectionUtil.isNotEmpty;
import static com.kuugen.modular.wechat.util.StrUtil.isNotBlank;
...
public abstract class WeixinSupport {
    ...
    protected String processRequest(HttpServletRequest request) {
        ...
        if (msgType.equals(ReqType.EVENT)) {
            ...
            if (eventType.equals(EventType.SUBSCRIBE)) {
             ...
             //扫描二维码事件
            }else if (eventType.equals(EventType.SCANCODE_WAITMSG)) {
            	 BaseEvent event = new BaseEvent();
                 
            	 msg=getQrcodeinfo(reqMap, event);
     ...
     * @return 响应消息对象
     */
    protected BaseMsg getQrcodeinfo(Map<String, String> reqMap, BaseEvent event) {
		String qrcode=reqMap.get("ScanResult");
    	return new TextMsg(qrcode);
	}
    /**
     * 处理取消关注事件，有需要时子类重写
     *
